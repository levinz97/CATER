...
class Merge_all:
    ...
    def merge_consist(self, start: int, end: int):
        ...
        output_path = './d_data/{}-{}.json'.format(start,end)
        for i in range(start,int(end-3),5):
            input_path = './d_data/merge/{}-{}.json'.format(str(i),str(int(i+4)))
            self.add_images_and_annotations(input_path)
            self.save(output_path)
            print(2*'>>>>>>>>>'+'.{}-{}.json '.format(str(i),str(int(i+4)))+'merge finish')
            print('success!!!')

    def merge_arbitrary(self, file1_start: int, file1_end: int, file2_start: int, file2_end: int):
        
        input_path1 = './merge_data/{}-{}.json'.format(str(file1_start),str(file1_end))
        input_path2 = './merge_data/{}-{}.json'.format(str(file2_start),str(file2_end))
        output_path = './merge_data/{}-{}.json+{}-{}.json'.format(file1_start,file1_end,file2_start,file2_end)
...
if __name__ == "__main__":
    ...
    #ma.merge_consist(start=5200, end=5264)

    # if some files are missed before, then use this code to merge them
    # e.g. if file1 = '5215-5240.json'
    ma.merge_arbitrary(file1_start=5200, file1_end=5264, file2_start= 5301, file2_end= 5341)